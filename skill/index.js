        ...
        dynamodb.getItem(params, function (err, data) {
            if (err) console.log(err, err.stack); // an error occurred
                ...
                if (nextWord === data.Item.answer.S) { // TODO: need to check if exists
                    ...
                    else if ((indexLength - currentPos) <= 1) {
                        vm.emit(':tell', originArray[currentPos + 1] + '. You did it!');
                        //no put to db
                    }
                    else if ((indexLength - currentPos) <= 2) {
                        vm.emit(':tell', originArray[currentPos + 1] + originArray[currentPos + 2] + '. You did it guys!');
                        //put to db
                    } else {
                        let params = {
                            Item: {
